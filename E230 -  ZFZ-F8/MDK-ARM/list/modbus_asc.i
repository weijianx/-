...
extern uint8_t Cur_Param[(((((((((0x00 + 0x02) + 0x01) + 0x01) + 0x04) + 0x04) + 0x04) + 0x04) + 0x04) + 0x04)];
extern uint8_t const User_Default_Param[(((((((((0x00 + 0x02) + 0x01) + 0x01) + 0x04) + 0x04) + 0x04) + 0x04) + 0x04) + 0x04)];
...
void MBASC_Fun04(void)
{
    ...
    if( (!(( ( (ReadAdrL <= 0x0E) && ((ReadAdrL + Register_Num) <= (0x0E + 2))) )&& ((0 != Register_Num) && (0 == (Register_Num & 0x01)) && (0 == (ReadAdrL & 0x01))))) || (ReadAdrH != UserPara.SlaveAddr))
    ...
    for(uint8_t k = 0; k < Register_Num; ReadAdrL += 2, k += 2)
    {
        switch (ReadAdrL)
        {
          case 0x00:
              ...
              Data_Buf = UserPara.RotateSta;

              break;
          case 0x02:
              Data_Buf = UserPara.DirSta;
            break;

          case 0x04:
              Data_Buf = UserPara.RotateSpeed;
            break;

          case 0x06:
              Data_Buf = UserPara.WorkTime;
            break;

          case 0x08:
              Data_Buf = UserPara.TotalPulse;
            break;
...
void MBASC_Fun05()
{
    ...
    if ((!((((ReadAdrL >= 0x50) && (ReadAdrL <= 0x54)))))|| (ReadAdrH != UserPara.SlaveAddr))
    {
        MBASC_SendErr((0x02));
        return;
    }
      ...
      return;
        ...
        switch (ReadAdrL)
        {
            case 0x50:
             ...
             else if((UARTx_RXBuff[4] == 0xFF) && (UARTx_RXBuff[5] == 0x00))
              {
            ...
            case 0x51:
               if ((UARTx_RXBuff[4] == 0x00) && (UARTx_RXBuff[5] == 0x00))
                  {
                    Flash_Write_MultiBytes(0x00, "\x02", 1);
                    ReadPara();
                    Flash_Write_MultiBytes(0x00, "\x01", 1);
                  }
               break;
    ...
    for (uint8_t k = 0; k < Register_Num; ReadAdrL++, k++)
    {
        switch (ReadAdrL)
        {
            ...
            case 0x39:
                ...
                if(uint16_tTemp != 0xFFFF)
                {
                    UserPara.WorkTime = uint16_tTemp;
                    long32Array(UserPara.WorkTime, uTemp);
                    Flash_Write_MultiBytes((((0x00 + 0x02) + 0x01) + 0x01), uTemp, 4);
                }
                break;
            ...
            case 0x3A:
                ...
                if(uint16_tTemp != 0xFFFF)
                {
                    ...
                    Flash_Write_MultiBytes((((((((0x00 + 0x02) + 0x01) + 0x01) + 0x04) + 0x04) + 0x04) + 0x04), uTemp, 4);
                }
                break;
...
# 719 "../APP/Src/modbus_asc.c"
            break;

        case 0x51E2:
            SendBuf[SendLen++] = 0xE2;
# 737 "../APP/Src/modbus_asc.c"
            break;

        case 0x51E3:
            SendBuf[SendLen++] = 0xE3;
            SoftwareVersionLen = 8;
...
# 773 "../APP/Src/modbus_asc.c"
            break;

        case 0x51E5:
            SendBuf[SendLen++] = 0xE5;
# 791 "../APP/Src/modbus_asc.c"
           break;
